...
/*
    chooseBeneficiary(): void {
      ...
      if (selectedAccount) {
        ...
        if (selectedAmount > parseFloat(this.userAccounts.find(account => account.numeroCuenta === this.selectedAccount)?.saldo)) {
...
export class TransferenciaComponent implements OnInit {
  ...
  fetchAccounts(usuario: string): void {
    this.accountService.getUserAccounts(usuario).subscribe(
      data => {
        ...
        if (this.userAccounts.length > 0) {
          this.selectedAccount = this.userAccounts[0].numeroCuenta;
          this.numeroIdentidad = this.userAccounts[0].numero_identidad; // Guardar numero_identidad
        }
      },
      error => {
        console.error('Error fetching accounts:', error);
  ...
  onAccountChange(event: Event): void {
    ...
    const selectedAccount = this.userAccounts.find(account => account.numeroCuenta === this.selectedAccount);
    if (selectedAccount) {
      this.numeroIdentidad = selectedAccount.numero_identidad; // Actualizar numero_identidad cuando cambie la cuenta seleccionada
    }
  }
